...
const fakeRecipe = {
  ...
  steps: [
    "Faire dorer les filets de poulets à la poêle",
...
describe("recipe CRUD", () => {
  ...
  after(done => {
    Model.remove({ test: true }).then(() => done());
  });
  ...
  it("select one", () => {
    ...
    return Model.findOne({ title: titleToLook }).then(recipe => {
  ...
  it("select by id", () => {
    return Model.findById(recipeOfWork._id).then(recipe => {
      expect(recipe).not.to.be.null;
...
describe("recipe base REST API", () => {
  ...
  after(done => {
    return Model.remove({ test: true }).then(() => done());
  });
